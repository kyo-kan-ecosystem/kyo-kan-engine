...
describe('StackTree', () => {
    ...
    it('should push and pop data on the current branch', () => {
        const tree = new StackTree();
        tree.now.push({ step: 1 });
        tree.now.push({ step: 2 });
        expect(tree.getBranchDepth()).to.equal(2);
        expect(tree.now.get()).to.deep.equal({ step: 2 });
        const popped = tree.pop();
        expect(popped).to.deep.equal({ step: 2 });
        expect(tree.now.get()).to.deep.equal({ step: 1 });
    });

    it('should set and switch branch IDs', () => {
        const tree = new StackTree();
        tree.now.push({ branch: 0 });
        tree.setBranchId(1);
        expect(tree.getBranchId()).to.equal(1);
        expect(tree.isEmptyNow()).to.be.true; // New branch is empty
        tree.now.push({ branch: 1 });
        expect(tree.now.get()).to.deep.equal({ branch: 1 });

        tree.setBranchId(0);
        expect(tree.getBranchId()).to.equal(0);
        expect(tree.now.get()).to.deep.equal({ branch: 0 });
    });
    ...
    it('should serialize and deserialize the tree state', () => {
        ...
        expect(newTree.now.get()).to.deep.equal({ id: 'a' });
        newTree.setBranchId(1);
        expect(newTree.now.get()).to.deep.equal({ id: 'b' });
    });
    ...
    describe('fork()', () => {
        ...
        it('should reflect changes in the fork back to the original tree (shared data)', () => {
            ...
            expect(tree.now.get()).to.deep.equal({ fromFork: true });
        });
        ...
        it('should create distinct branches for each fork call without an ID', () => {
            ...
            expect(fork1.now.get()).to.deep.equal({ id: 1 }); // Should not be affected by fork2
            expect(fork2.now.get()).to.deep.equal({ id: 2 });
        });
    });
    ...
    it('should remove a branch', () => {
        const tree = new StackTree();
        tree.setBranchId(1);
        tree.now.push({ data: 'to be removed' });
